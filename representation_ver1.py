...
def output_representation(args, opts):
    ...
    for i in range(num_batches):
        print("Saving batch", i, "of", num_batches, "to disk...")
        start_idx = i * batch_size
        end_idx = (i + 1) * batch_size
        np.save(os.path.join(opts.checkpoint, f"embeddings_{i}.npy"), embeddings[start_idx:end_idx])
        np.save(os.path.join(opts.checkpoint, f"labels_{i}.npy"), labels[start_idx:end_idx])
