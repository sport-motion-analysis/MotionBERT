...
def _output_representation(test_loader, backbone_model):
    ...
    with torch.no_grad():
        for idx, (batch_input, batch_gt) in tqdm(enumerate(test_loader)):
            ...
            # save output as with index at "chekcpoint/representation/" as .npy
            # if doesn't exist, create the directory
            if not os.path.exists(os.path.join(opts.checkpoint, "representation")):
                os.makedirs(os.path.join(opts.checkpoint, "representation"))
            np.save(os.path.join(opts.checkpoint, "representation", "feature"+str(idx)+".npy"), output)
            output_all.append(output)
            # output_all = torch.cat((output_all, output), dim=0)
            # and ground truth gt
            batch_gt = batch_gt.cpu()
            np.save(os.path.join(opts.checkpoint, "representation", "label"+str(idx)+".npy"), batch_gt)
